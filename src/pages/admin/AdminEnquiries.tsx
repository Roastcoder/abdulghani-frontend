...
const AdminEnquiries = () => {
  ...
  const updateStatus = async (id: number, status: string) => {
    ...
    if (res.ok) { toast({ title: "Status updated" }); queryClient.invalidateQueries({ queryKey: ["enquiries"] }); }
  };
  ...
  const deleteEnquiry = async (id: number) => {
    ...
    if (res.ok) { toast({ title: "Enquiry deleted" }); queryClient.invalidateQueries({ queryKey: ["enquiries"] }); }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">Manage customer enquiries</p>
      </div>
            ...
            <div key={e.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${statusStyles[e.status]}`}>{e.status}</span>
                  ...
                  {["new", "contacted", "closed"].map(s => (
                    <button key={s} onClick={() => updateStatus(e.id, s)}
                      className={`text-xs font-medium px-3 py-1.5 rounded-lg border transition-all ${
                        e.status === s ? statusStyles[s] : "border-gray-200 text-gray-400 hover:text-gray-700 hover:bg-gray-50"
                      }`}>
