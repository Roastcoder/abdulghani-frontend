...
const AdminFAQ = () => {
  ...
  const validate = () => {
    ...
    if (!editing.question.trim()) e.question = "Required";
    if (!editing.answer.trim()) e.answer = "Required";
    setErrors(e);
  ...
  const openNew = () => {
    setEditing({ ...emptyForm, sort_order: (faqs as any[]).length + 1 });
    setErrors({});
    setIsNew(true);
    setOpen(true);
  };
  ...
  const openEdit = (faq: any) => {
    setEditing({ id: faq.id, question: faq.question, answer: faq.answer, sort_order: faq.sort_order });
    setErrors({});
    setIsNew(false);
    setOpen(true);
  };
    ...
    <div className="space-y-6">
              ...
              <div key={faq.id} className="flex items-start gap-4 px-6 py-5 hover:bg-gray-50 transition-colors group">
                ...
                <div className="flex items-center gap-1 shrink-0 mt-0.5">
                  <button onClick={() => openEdit(faq)}
                    className="w-8 h-8 rounded-lg border border-gray-200 hover:border-primary/30 hover:bg-primary/5 text-gray-400 hover:text-primary flex items-center justify-center transition-all">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleDelete(faq.id)}
                    className="w-8 h-8 rounded-lg border border-gray-200 hover:border-red-200 hover:bg-red-50 text-gray-400 hover:text-red-500 flex items-center justify-center transition-all">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl bg-white">
          <DialogHeader>
            <DialogTitle className="text-gray-900 font-bold">{isNew ? "Add FAQ" : "Edit FAQ"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            <div className="space-y-1.5">
              <label className="text-gray-600 text-xs font-semibold uppercase tracking-wider flex items-center gap-1">
                Question {errors.question && <span className="text-red-500 normal-case font-normal">— {errors.question}</span>}
              </label>
              <input
                value={editing.question}
                onChange={e => { setEditing({ ...editing, question: e.target.value }); setErrors(er => ({ ...er, question: undefined })); }}
                placeholder="e.g. What types of equipment do you offer?"
                className={`${inputCls} ${errors.question ? "border-red-300 focus:border-red-400 focus:ring-red-100" : ""}`}
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-gray-600 text-xs font-semibold uppercase tracking-wider flex items-center gap-1">
                Answer {errors.answer && <span className="text-red-500 normal-case font-normal">— {errors.answer}</span>}
              </label>
              <textarea
                value={editing.answer}
                onChange={e => { setEditing({ ...editing, answer: e.target.value }); setErrors(er => ({ ...er, answer: undefined })); }}
                placeholder="Write a clear and helpful answer..."
                rows={5}
                className={`${inputCls} resize-none ${errors.answer ? "border-red-300 focus:border-red-400 focus:ring-red-100" : ""}`}
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-gray-600 text-xs font-semibold uppercase tracking-wider">Sort Order</label>
              <input
                type="number"
                ...
                min={0}
              />
              <p className="text-gray-400 text-xs">Lower number = appears first</p>
            </div>
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
              <button onClick={() => setOpen(false)}
                className="px-4 py-2.5 rounded-xl text-sm font-medium text-gray-500 hover:text-gray-800 border border-gray-200 hover:bg-gray-50 transition-all">
                Cancel
              </button>
              <button onClick={handleSave}
                className="px-5 py-2.5 rounded-xl text-sm font-semibold bg-primary hover:bg-primary/90 text-primary-foreground transition-all shadow-sm">
