...
const AdminProducts = () => {
  ...
  const req = hi ? "आवश्यक" : "Required";

  const validate = () => {
    const e: Partial<ProductForm> = {};
    if (!editing?.name) e.name = req;
    if (!editing?.category) e.category = req;
    if (!editing?.short_desc) e.short_desc = req;
    if (!editing?.image_url) e.image_url = req;
    setErrors(e);
  ...
  const handleSave = async () => {
    ...
    if (res.ok) {
      toast({ title: isNew ? (hi ? "उत्पाद बनाया गया" : "Product created") : (hi ? "उत्पाद अपडेट हुआ" : "Product updated") });
      queryClient.invalidateQueries({ queryKey: ["products"] });
      setOpen(false);
    } else {
      toast({ title: hi ? "त्रुटि" : "Error", description: hi ? "सहेजने में विफल" : "Failed to save", variant: "destructive" });
    }
  };

  const handleDelete = async (id: string) => {
    if (!confirm(hi ? "इस उत्पाद को हटाएं?" : "Delete this product?")) return;
    const res = await fetch(`${API_BASE_URL}/api/products`, {
      method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }),
    });
    if (res.ok) { toast({ title: hi ? "उत्पाद हटाया गया" : "Product deleted" }); queryClient.invalidateQueries({ queryKey: ["products"] }); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{hi ? "उत्पाद" : "Products"}</h1>
          <p className="text-gray-400 text-sm mt-1">{hi ? "अपनी उत्पाद सूची प्रबंधित करें" : "Manage your product catalog"}</p>
        </div>
        <button onClick={openNew} className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-sm">
          <Plus className="w-4 h-4" /> {hi ? "उत्पाद जोड़ें" : "Add Product"}
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-gray-100 rounded-2xl py-16 text-center shadow-sm">
          ...
          <p className="text-gray-400 text-sm">{hi ? "अभी तक कोई उत्पाद नहीं" : "No products yet"}</p>
        </div>
      ) : (
            ...
            {products.map((p: any) => (
              <div key={p.id} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                <div className="w-14 h-14 rounded-xl overflow-hidden bg-gray-100 shrink-0 border border-gray-100">
                  {p.image_url ? <img src={resolveImg(p.image_url)} alt={p.name} className="w-full h-full object-cover" />
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => openEdit(p)} className="w-8 h-8 rounded-lg border border-gray-200 hover:border-primary/30 hover:bg-primary/5 text-gray-400 hover:text-primary flex items-center justify-center transition-all"><Pencil className="w-3.5 h-3.5" /></button>
                  <button onClick={() => handleDelete(p.id)} className="w-8 h-8 rounded-lg border border-gray-200 hover:border-red-200 hover:bg-red-50 text-gray-400 hover:text-red-500 flex items-center justify-center transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white">
          <DialogHeader>
            <DialogTitle className="text-gray-900 font-bold">{isNew ? (hi ? "उत्पाद जोड़ें" : "Add Product") : (hi ? "उत्पाद संपादित करें" : "Edit Product")}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="space-y-4 mt-2">
              <div className="grid grid-cols-2 gap-3">
                <F label={hi ? "उत्पाद ID" : "Product ID"}>
                ...
                <F label={hi ? "श्रेणी *" : "Category *"} error={errors.category}>
                  <input value={editing.category} onChange={e => setEditing({ ...editing, category: e.target.value })} className={`${inputCls} ${errors.category ? "border-red-300" : ""}`} />
                </F>
              </div>
              <F label={hi ? "उत्पाद नाम *" : "Product Name *"} error={errors.name}>
                <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })} className={`${inputCls} ${errors.name ? "border-red-300" : ""}`} />
              </F>
              <F label={hi ? "संक्षिप्त विवरण *" : "Short Description *"} error={errors.short_desc}>
                <input value={editing.short_desc} onChange={e => setEditing({ ...editing, short_desc: e.target.value })} className={`${inputCls} ${errors.short_desc ? "border-red-300" : ""}`} />
              </F>
              <F label={hi ? "पूरा विवरण" : "Full Description"}>
              ...
              <F label={hi ? "विशेषताएं (प्रति पंक्ति एक)" : "Features (one per line)"}>
              ...
              <ImageUpload label={`${hi ? "उत्पाद छवि" : "Product Image"} — ${editing.name || (hi ? "नया उत्पाद" : "New Product")}`} required
                value={editing.image_url}
                onChange={url => { setEditing({ ...editing, image_url: url }); setErrors(e => ({ ...e, image_url: undefined })); }} />
              {errors.image_url && <p className="text-red-500 text-xs -mt-1">{hi ? "उत्पाद छवि आवश्यक है" : "Product image is required"}</p>}
              <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
                ...
                <button onClick={handleSave} className="px-5 py-2.5 rounded-xl text-sm font-semibold bg-primary hover:bg-primary/90 text-primary-foreground transition-all shadow-sm">
                  {isNew ? (hi ? "उत्पाद बनाएं" : "Create Product") : (hi ? "परिवर्तन सहेजें" : "Save Changes")}
                </button>
              </div>
            </div>
          )}
